...
async function main() {
  ...
  // 1. Test markets=buff
  await testParam('markets=buff', 
    `https://www.steamwebapi.com/steam/api/float/assets?key=${apiKey}&appid=730&market_hash_name=${encodedName}&markets=buff`);

  console.log('Sleeping 35s to respect rate limit...');
  await sleep(35000);

  // 2. Test market=buff
  await testParam('market=buff', 
    `https://www.steamwebapi.com/steam/api/float/assets?key=${apiKey}&appid=730&market_hash_name=${encodedName}&market=buff`);
}
